...
bool IntSet::isSubsetOf(const IntSet& otherIntSet) const
{
    ...
    {
        if(otherIntSet.contains(this->data[i]) == false){
          return false;
        }
    }
...
IntSet IntSet::unionWith(const IntSet& otherIntSet) const
{
    ...
    {
         if(unionIntSet.contains(otherIntSet.data[i]) == false)
         {
...
{
    IntSet subtractedIntSet = *this;

    for(int i = 0; i < used; i++)
    {
        if(otherIntSet.contains(data[i]))
        {
          subtractedIntSet.remove(otherIntSet.data[i]);
        }
    }

    return subtractedIntSet;
}
    ...
    if(contains(anInt) == true)
    {
        for(int i = 0; i < size(); i++)
        {
           if(data[i] == anInt)
           {
               if(i + 1 <= size())
               {
                   int numbersLeftToCheck = used - i;

                   for(int j = 0; j < numbersLeftToCheck; j++){
                       data[i] = data[i + 1];
                       i++;
                   }
                   used--;
